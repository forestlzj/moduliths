...
public class PersistentApplicationEventMulticaster extends AbstractApplicationEventMulticaster
	...
	public void afterSingletonsInstantiated() {
		...
		listeners.stream()
				.forEach(it -> it.addCallback(new CompletionRegisteringSynchronizationCallback(eventPublicationRegistry, it)));
	...
	/**
	 * A {@link SynchronizationCallback} that marks the publication for the given {@link TransactionalApplicationListener}
	 * as completed.
	 *
	 * @author Oliver Drotbohm
	 */
	@RequiredArgsConstructor
	private class CompletionRegisteringSynchronizationCallback implements SynchronizationCallback {

		private final EventPublicationRegistry registry;
		private final TransactionalApplicationListener<?> listener;

		/*
		 * (non-Javadoc)
		 * @see org.springframework.transaction.event.TransactionalApplicationListener.SynchronizationCallback#postProcessEvent(org.springframework.context.ApplicationEvent, java.lang.Throwable)
		 */
		@Override
		public void postProcessEvent(ApplicationEvent event, Throwable ex) {

			String id = listener.getListenerId();

			if (ex == null) {
				registry.markCompleted(getEventToPersist(event), PublicationTargetIdentifier.of(id));
				return;
			}

			LOG.info("Failure during transaction event processing of {}. {}", event, ex.getMessage());
		}
	}
